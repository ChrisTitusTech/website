...
for (const path of ["/", "/my-ai-workflow/", "/categories/linux/", "/live-streams/"]) {
  test(`primary page ${path} has no serious accessibility violations`, async ({ page }) => {
...
test("article exposes navigation and interactions", async ({ page, isMobile }) => {
  ...
  await expect(page.getByRole("link", { name: /continue the discussion on github/i })).toBeVisible();
});

test("numeric article headings do not abort site enhancements", async ({ page }) => {
  const errors: Error[] = [];
  page.on("pageerror", (error) => errors.push(error));
  await page.goto("/windows-24h2/");
  const toc = page.locator("[data-responsive-toc]");
  if (!(await toc.evaluate((element) => element.hasAttribute("open")))) await toc.locator("summary").click();
  await expect(page.locator('[data-toc] a[href="#24h2-livestream"]')).toBeVisible();
  await page.locator("[data-comments]").scrollIntoViewIfNeeded();
  await expect(page.getByRole("link", { name: /continue the discussion on github/i })).toBeVisible();
  expect(errors).toEqual([]);
});

test("responsive article navigation follows viewport changes", async ({ page, isMobile }) => {
...
test("live archive paginates and validates player ids", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: /older/i })).toHaveAttribute("href", "/live-streams/page/2/");
...
test("newsletter retains required subscription contract", async ({ page }) => {
  ...
  await expect(form).toHaveAttribute("action", "https://newsletter.christitus.com/subscribe");
  await expect(form.locator('input[name="list"]')).toHaveValue("BdlkPnowdu7nhx6niCzQUw");
  await expect(form.locator(".g-recaptcha")).toHaveAttribute("data-sitekey", /.+/);
  await form.scrollIntoViewIfNeeded();
  await expect(page.locator("[data-recaptcha-status]")).toContainText("Spam protection is unavailable");
});
...
test("search returns generated index results", async ({ page }) => {
  ...
  await expect(page.locator("[data-search-results] article").first()).toBeVisible();
});
...
test("downloads retain an intent-driven store fallback", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: /browse the ctt store directly/i })).toHaveAttribute("href", "https://www.cttstore.com/");
  await button.click();
  await expect(button).toHaveText("Store unavailable - try again");
  await expect(page.getByRole("link", { name: /browse the ctt store directly/i })).toBeVisible();
});

test("known player states render and unknown ids redirect", async ({ page, isMobile }) => {
  await page.goto("/live-streams/player/?v=wf9rLEjWmPE");
  await expect(page.getByRole("heading", { level: 1 })).toContainText("Taking a break");
  await expect(page.getByRole("link", { name: "Watch on YouTube" })).toHaveAttribute("href", "https://www.youtube.com/watch?v=wf9rLEjWmPE");
  ...
  if (!isMobile) {
    const fits = await page.locator("#player-wrapper").evaluate((wrapper) => {
      ...
      return chat.getBoundingClientRect().bottom <= wrapper.getBoundingClientRect().bottom + 1;
    });
    expect(fits).toBe(true);
  }
  await page.goto("/live-streams/player/?v=hF3dAcTSivs");
  await expect(page.getByRole("heading", { level: 1 })).not.toHaveText("Live Stream");
...
test("mobile navigation opens with an accessible control", async ({ page, isMobile }) => {
