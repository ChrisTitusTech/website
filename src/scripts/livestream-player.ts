...
type ChatComment = {
  ...
  message?: {
    body?: string;
    user_color?: string;
    user_badges?: Array<{ _id: string; version: string }>;
    fragments?: Array<{ text?: string; emoticon?: { emoticon_id?: string } }>;
  };
};

const dataElement = document.querySelector<HTMLScriptElement>("#stream-data");
const streams: Stream[] = dataElement
  ? JSON.parse(dataElement.textContent ?? "[]")
  : [];
const videoId = new URLSearchParams(location.search).get("v") ?? "";
const stream = streams.find((item) => item.videoId === videoId);
if (!stream || !/^[A-Za-z0-9_-]{6,16}$/.test(videoId))
  location.replace("/live-streams/");
...
if (stream) {
  ...
  const fallback =
    document.querySelector<HTMLAnchorElement>("#youtube-fallback");
  if (fallback)
...
function renderMessage(comment: ChatComment, suppressScroll = false) {
  ...
  for (const fragment of comment.message?.fragments ?? [
    ...
    if (!emoteId) {
      body.append(document.createTextNode(fragment.text ?? ""));
      continue;
    }
    const image = document.createElement("img");
    image.className = "chat-emote";
    image.alt = fragment.text ?? "emote";
    image.title = image.alt;
    image.src =
      emoteMap.get(emoteId) ??
      `https://static-cdn.jtvnw.net/emoticons/v2/${encodeURIComponent(emoteId)}/default/dark/1.0`;
...
function pointerAt(time: number) {
  ...
  while (low < high) {
    ...
    if (comments[middle].content_offset_seconds <= time) low = middle + 1;
    else high = middle;
  }
  return low;
}
function seekChat(time: number) {
  ...
  pointer = pointerAt(time);
  for (let index = Math.max(0, pointer - 50); index < pointer; index += 1)
    renderMessage(comments[index], true);
  container.scrollTop = container.scrollHeight;
  lastTime = time;
}
function syncChat() {
  const time = player?.getCurrentTime();
  if (time === undefined || Number.isNaN(time) || time < 0) return;
  if (lastTime >= 0 && Math.abs(time - lastTime) > 3) {
    seekChat(time);
    return;
  }
  lastTime = time;
  while (
    ...
    comments[pointer].content_offset_seconds <= time
  )
    renderMessage(comments[pointer++]);
  const status = document.querySelector<HTMLElement>("#chat-status");
  if (status) status.textContent = `${pointer}/${comments.length}`;
}

const messages = document.querySelector<HTMLElement>("#chat-messages");
const scrollButton =
  document.querySelector<HTMLButtonElement>("#chat-scroll-btn");
messages?.addEventListener("scroll", () => {
  autoScroll =
    messages.scrollHeight - messages.scrollTop - messages.clientHeight < 80;
  if (scrollButton) scrollButton.hidden = autoScroll;
});
...
if (stream?.hasChatReplay)
  fetch(`/chats/${encodeURIComponent(videoId)}.json`)
    .then((response) => {
      if (!response.ok) throw new Error("Chat replay unavailable");
      return response.json();
    })
    .then((data) => {
      for (const emote of data.embeddedData?.firstParty ?? [])
        if (emote.id && emote.data)
          emoteMap.set(
            emote.id,
            `data:image/${emote.imageType ?? "png"};base64,${emote.data}`,
          );
      for (const badge of data.embeddedData?.twitchBadges ?? [])
        for (const [version, value] of Object.entries<any>(
          badge.versions ?? {},
        ))
          if (value.bytes)
            badgeMap.set(
              `${badge.Name}/${version}`,
              `data:image/${value.imageType ?? "png"};base64,${value.bytes}`,
            );
      comments = (data.comments ?? [])
        .filter((comment: ChatComment) =>
          Number.isFinite(comment.content_offset_seconds),
        )
        .sort(
          (left: ChatComment, right: ChatComment) =>
            left.content_offset_seconds - right.content_offset_seconds,
        );
      const chat = document.querySelector<HTMLElement>("#chat-col");
  ...
  player = new window.YT.Player("yt-player", {
    ...
    events: {
      onStateChange(event: { data: number }) {
        if (event.data === window.YT.PlayerState.PLAYING && timer === undefined)
          timer = window.setInterval(syncChat, 500);
        else if (
