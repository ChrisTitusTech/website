...
function syncThemeControl() {
  ...
  document.querySelector<HTMLIFrameElement>("iframe.utterances-frame")?.contentWindow?.postMessage(
...
document.querySelectorAll<HTMLImageElement>("img[data-cf-image]").forEach((image) => {
...
const responsiveToc = document.querySelector<HTMLDetailsElement>("[data-responsive-toc]");
const tocMedia = matchMedia("(max-width: 940px)");
const syncResponsiveToc = (mobile: boolean) => responsiveToc?.toggleAttribute("open", !mobile);
syncResponsiveToc(tocMedia.matches);
tocMedia.addEventListener("change", (event) => syncResponsiveToc(event.matches));

const progress = document.querySelector<HTMLElement>("[data-reading-progress]");
if (progress) {
  const update = () => {
    const range = document.documentElement.scrollHeight - innerHeight;
    progress.style.setProperty("--reading-progress", `${range > 0 ? Math.min(100, scrollY / range * 100) : 0}%`);
  };
...
document.querySelectorAll<HTMLElement>(".article-body :is(h2,h3)[id]").forEach((heading) => {
...
document.querySelector<HTMLButtonElement>("[data-copy-page]")?.addEventListener("click", async (event) => {
...
if (tocLinks.length && "IntersectionObserver" in window) {
  const observer = new IntersectionObserver((entries) => {
    for (const entry of entries) if (entry.isIntersecting) {
      tocLinks.forEach((link) => link.toggleAttribute("aria-current", link.hash === `#${entry.target.id}`));
    }
...
function loadUtterances() {
  ...
  script.setAttribute("theme", root.dataset.theme === "light" ? "github-light" : "github-dark");
...
if (comments && "IntersectionObserver" in window) {
  const observer = new IntersectionObserver((entries) => {
    if (entries.some((entry) => entry.isIntersecting)) { loadUtterances(); observer.disconnect(); }
  }, { rootMargin: "500px" });
  observer.observe(comments);
}

function loadAds() {
  if (!document.querySelector("[data-ad-slot]") || document.querySelector("script[data-google-ads]")) return;
  const script = document.createElement("script");
  script.async = true;
  script.dataset.googleAds = "true";
  script.crossOrigin = "anonymous";
  script.src = "https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-6679678361432308";
...
document.querySelector<HTMLButtonElement>("[data-shopify-load]")?.addEventListener("click", async (event) => {
  const button = event.currentTarget as HTMLButtonElement;
  const host = button.closest<HTMLElement>("[data-shopify-products]");
  if (!host) return;
  button.disabled = true; button.textContent = "Loading store...";
  const script = document.createElement("script");
  ...
  script.addEventListener("load", async () => {
    try {
      ...
      const client = ShopifyBuy.buildClient({ domain: "1efdc2-1a.myshopify.com", storefrontAccessToken: "d3832fa1046e2ebd19d3bfd837bf5eb3" });
        ...
        options: {
          product: {
            ...
            styles: {
              ...
              button: { "background-color": "#347eaa", "font-weight": "bold", ":hover": { "background-color": "#28688e" } },
            },
          },
        },
      });
  ...
  script.addEventListener("error", () => { button.disabled = false; button.textContent = "Store unavailable - try again"; }, { once: true });
  document.head.append(script);
});
